...
class DrivingBenchmark(object):
    ...
    def _run_navigation_episode(
        ...
        while (current_timestamp - initial_timestamp) < (time_out * 1000) and not success:
            ...
            delta_x, delta_y = abs(current_x - last_x), abs(current_y - last_y)
            # if stuck, break and set timeout to save evaluation time
            if delta_x<0.01 and delta_y<0.01:
                if stuck==0:
                    stuck=current_timestamp
                else:
                    if current_timestamp-stuck>6*1000:
                        stuck=0
                        # Demo Video modification 4
                        break
            else:
                stuck=0
