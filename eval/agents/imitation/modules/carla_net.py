...
from transformers import ViTModel, ViTFeatureExtractor, ViTMAEModel, AutoFeatureExtractor, DetrModel, DetrFeatureExtractor
...
class CarlaViT(nn.Module):
    def __init__(self, name, dropout_vec=None):
        super(CarlaViT, self).__init__()

        self.name = name
        print(f'\n{name}\n')
        assert name in {'vit', 'mae', 'detr'}
        ...
        if name=='vit':
            ...
            self.feature_extractor = DetrFeatureExtractor.from_pretrained(
                ...
                cache_dir='./cache', do_resize=False)
    ...
    def forward(self, img, speed):
        ...
        output = self.feature_extractor(images=img.cpu(), return_tensors="pt")
        output = output['pixel_values'].cuda()
        
        outputs = self.model(pixel_values=output)

        if self.name == 'vit':
            img = outputs.pooler_output
        elif self.name=='mae' or self.name=='detr':
